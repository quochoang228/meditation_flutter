...
import 'base_observe.dart';

/// The abstract [BaseUseCase] to be implemented by all usecases.
/// [T] Is the type to be returned by the [BaseUseCase] to the [Presenter]
/// [Params] Is the object passed to the usecase containing all the needed parameters
/// for the [BaseUseCase]
///
/// The [BaseUseCase] represents a business-level process. It should be written purely
...
///
/// Dependencies used by the [BaseUseCase] must be injected by the [Presenter]. The [BaseUseCase]
/// is essentially an `Stream` managing class. When the [execute()] function is triggered
/// by the [BaseUseCase], an `Stream` is built using the `buildUseCaseStream()` method, subscribed to
...
/// of the [BaseUseCase] are defined. This method will return the `Stream` to be subscribed to, and will fire events to
...
abstract class BaseUseCase<T, Params> {
  ...
  /// by the [BaseUseCase] to retrieve the appropriate data from the repository
...
abstract class CompletableUseCase<Params> extends BaseUseCase<void, Params?> {
  @override
